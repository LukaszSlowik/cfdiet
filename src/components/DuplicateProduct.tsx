...
let disableClass = userid !== product.userId ? "pointer-events-none opacity-40" : ""
  const { data, isLoading, isError, mutate } = useMutation({
    ...
    onSettled: () => {
      ...
      router.push(`/products/productsList?q=${encodedSearchQuery}&global=false`)
    },
  });
  ...
  return (
    ...
    <span className="invisible group-hover:visible opacity-0 group-hover:opacity-100 transition border-slate-50 ring-[1px] ring-green-700 text-zinc-50 p-2 rounded mt-1 absolute top-full whitespace-nowrap  bg-slate-700 "> Duplicate as yours </span>
    </div>
  );
}
